...
scan_params=unique(stg_table(:,3))'; % all parameters that have actual transitions
% or by selecting given nodes by their names: scan_params=find(ismember(nodes,{'cdc25b','atm_atr'}));
scan_params_up_down=num2cell(repelem([1 2],numel(scan_params),1),2)'; % both up and down rates
...
[scan_par_table,scan_par_inds,parscan_matrix]=fcn_generate_parscan_matrix(scan_params,scan_params_up_down,nodes,sampling_types{1},parscan_min_max,n_steps);
...
[stationary_state_vals_onedimscan,stationary_node_vals_onedimscan,stationary_state_inds_scan]=fcn_onedim_parscan(stg_table,transition_rates_table,...
                                                            x0,nodes,parscan_matrix,scan_par_inds);
...
% scatter plots of parameters (to see their distribution)
% subplot_inds=fcn_norep_perms(1:size(all_par_vals_lhs,2));
% for k=1:size(subplot_inds,1)
%     subplot(round(sqrt(size(subplot_inds,1))),round(sqrt(size(subplot_inds,1))),k); 
%     scatter(all_par_vals_lhs(:,subplot_inds(k,1)),all_par_vals_lhs(:,subplot_inds(k,2)),0.6); 
%     if strfind(sampling_type,'log'); set(gca,'xscale','log'); set(gca,'yscale','log'); end
% end
...
% PLOT
fcn_multidim_parscan_scatterplot(var_ind,all_par_vals_lhs,scan_values,scan_params,scan_params_up_down,nodes,sampling_type,param_settings) % xlim_vals,

%% calculating & plotting (heatmap) correlations between variables and pars-vars linear/logist regression
...
% if we have already calculated <sobol_sensit_index>, provide it as input and only plot
