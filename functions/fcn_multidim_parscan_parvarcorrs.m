...
heatmap(corr_matr(var_selection(1:end-1),var_selection(2:end)),nodes(var_selection(2:end)),nodes(var_selection(1:end-1)),'%0.2f','TickAngle',90,'Colormap','redblue',... % 
    'MinColorValue',-1,'MaxColorValue',1,'GridLines','-','FontSize',num_size_plot,'ShowAllTicks',true,'NaNColor',[1 1 1]); set(gca,'FontSize',fontsize)
...
for k=1:size(varcorr_plot_indices,1)
   subplot(n_row_plot,n_col_plot,k); scatter(scan_values(:,varcorr_plot_indices(k,1)),scan_values(:,varcorr_plot_indices(k,2))); 
   xlim([0 1]); ylim([0 1]); xlabel(strrep(nodes{varcorr_plot_indices(k,1)},'_','\_'), 'FontSize', fontsize_labels ); 
    ylabel(strrep(nodes{varcorr_plot_indices(k,2)},'_','\_'),'FontSize', fontsize_labels );
...
r_squared(k,par_c)=1-SSresid/SStotal; 
slope_intercept{k,par_c}=p;
...
heatmap(val_to_plot,predictor_names,nodes(sel_nodes),'%0.2f','TickAngle',90,'Colormap','redblue',...
...
set(gca,'xticklabel',strrep(predictor_names,'_','\_')); 
