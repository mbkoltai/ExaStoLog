function [y_data,y_init_pred,init_error]=fcn_param_fitting_data_initguess_error(state_var_flag,x0,stg_table,data_param_vals,init_par_vals,...
...
y_init_pred=fcn_calc_init_stat_nodevals(x0,split_calc_inverse(fcn_build_trans_matr(stg_table,...
...
y_data=split_calc_inverse(fcn_build_trans_matr(stg_table,transition_rates_table_optim,''),stg_sorting_cell,transition_rates_table_optim,x0);
y_data=full(y_data(y_data>0));
% prediction with init guess
y_init_pred=split_calc_inverse(fcn_build_trans_matr(stg_table,...
		fcn_trans_rates_table(nodes,'uniform',[],[],predictor_names,init_par_vals),''),...
		stg_sorting_cell,transition_rates_table_optim,x0);
